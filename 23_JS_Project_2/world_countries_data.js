...
function createBarGraph(data, graphId) {
  ...
  const maxVal = Math.max(
    ...data.map((item) => item.population || item.speakers)
  );
  data.forEach((item, index) => {
    const bar = document.createElement("div");
    bar.className = "bar";
    bar.style.height = `${
      ((item.population || item.speakers) / maxVal) * 100
    }%`;
    bar.innerHTML = `${item.country || item.language}<br>${
      item.population || item.speakers
    }`;
